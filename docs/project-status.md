...
| 组件 | 状态 | 验证结果 |
|:---|:---|:---|
| React 19 + Vite 6 + TypeScript 5.5 | ✅ | 生产构建通过（260KB JS / 12KB CSS gzip） |
...
| 路由系统（React Router v6） | ✅ | `/` 首页、`/calendar`、`/tasks`、`/pomodoro` |
...
| 组件 | 状态 | 说明 |
|:---|:---|:---|
| NestJS 11 模块化架构 | ✅ | AppModule 挂载 5 个子模块 |
...
| 模块骨架 | ✅ | AuthModule / CalendarModule / TaskModule / PomodoroModule / SharedModule |
...
| DTO | ✅ | Event（Create/Update/Response）、Task（Create/Update/Response/Move）、Pomodoro（Create/End/Response/Stats） |
| 接口 | ✅ | `JwtPayload`、`OpenClawRequest/Response/Event`、`GlassTheme` |
| 编译 | ✅ | `tsc` 生成 `dist/` |
...
| OpenClaw Gateway | ⏸️ 注释 | 镜像未发布到公开仓库，Phase 2 通过本地 npm 安装 |
...
| `pull access denied for openclaw/gateway` | 镜像不存在于公开仓库 | 注释掉 docker-compose.yml 中的 openclaw 服务 |
...
```bash
# 2026-04-22
$ pnpm install          # ✅ 成功，851 个包，bcrypt/Prisma 编译通过
$ pnpm -F @phd/shared-types build   # ✅ tsc 零错误
$ pnpm -F @phd/frontend build       # ✅ Vite 1.71s，260KB JS + 12KB CSS
...
# ✅ AppModule / AuthModule / CalendarModule / TaskModule / PomodoroModule / ThrottlerModule 全部初始化
...
pnpm -F @phd/frontend build  → ✅ 零错误（333KB JS + 15KB CSS）
```
...
| 优先级 | 模块 | 核心工作 |
|:---|:---|:---|
| 高 | **认证系统** | JWT Access/Refresh Token、注册/登录页面、API 守卫 |
| 高 | **AI 助手集成** | OpenClaw WebSocket Client、SSE 流式对话 |
| 中 | **笔记系统** | TipTap 编辑器、Markdown 存储、RAG 向量索引 |
| 中 | **文献管理** | Zotero MCP Server 集成、PDF 上传与解析 |
| 低 | **工程优化** | 代码分割、单元测试、E2E 测试、CI/CD |
...
| OpenClaw Docker 镜像不存在 | Phase 1 无法通过 Docker 启动 OpenClaw | Phase 2 改为本地 `npm install openclaw` 方式运行 |
...
- [x] Docker Compose `pnpm docker:up` 一键启动 PostgreSQL + Redis
- [x] Prisma Migrate 成功执行，数据库表结构就绪
- [ ] 前端可访问 `/calendar`、`/tasks`、`/pomodoro`，UI 采用液态玻璃风格
- [x] 后端 Swagger 文档在 `/api/docs` 可访问，三个模块 API 可调用 ✅
- [x] **任务看板支持拖拽排序，乐观更新策略正常工作** ✅ 已完成
- [ ] FullCalendar 日历支持事件 CRUD 和重复规则
- [ ] 番茄钟计时器可正常开始/结束，数据持久化到后端
- [ ] JWT 认证保护敏感 API，前端带 Token 请求
- [x] `packages/shared-types` 无类型漂移，前后端编译通过
