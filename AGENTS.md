...
ResearchOS（科研生活助手）是一个面向科研工作者和深度知识工作者的**本地优先、AI 原生**个人科研工作台。项目直连 LLM API（Kimi Coding / OpenAI 兼容），通过 Function Calling 查询用户真实数据，构建覆盖文献管理、日程规划、实验记录、论文写作、番茄钟分析和 AI 助手对话的完整科研工作流。
...
**当前状态**: Phase 1 + Phase 2 核心模块全部跑通。覆盖文献管理、日程规划、实验记录、论文写作、番茄钟分析、AI 助手对话、RAG 语义检索、跨模块联动的完整科研工作流。
...
├── research-home-demo.html              # 首页静态 Demo（液态玻璃 UI）
└── AGENTS.md                       # 本文件
```
